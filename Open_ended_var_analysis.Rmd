...
library(scales)  # <— load this

ggplot(df_prop, aes(x = department, y = Prop, fill = all_laws)) +
  geom_col(width = 0.85, color = "grey20", linewidth = 0.2) +
  scale_y_continuous(labels = scales::label_percent(accuracy = 1)) +  # or percent_format()
  coord_flip() +
  labs(title = "Legal Bases for Personal Data Processing by Department",
    x = "County Department",
...
ggplot(law_counts, aes(x = reorder(response, n), y = n)) +
  ...
  coord_flip() +
  labs(
    title = "Who makes decisions on data collection, processing, storage and transmission",
    x = "Decision Maker",
    y = "Mentions"
  ) +
  theme_minimal()
...
dd_long <- dept_combined %>%
  separate_rows(all_laws, sep = ",") %>%
  mutate(all_laws = str_trim(all_laws)) %>%
  filter(!str_detect(all_laws, "None")) %>%
  filter(all_laws != "")
...
ggplot(df_prop, aes(x = department, y = Prop, fill = all_laws)) +
  ...
  scale_y_continuous(labels = scales::label_percent(accuracy = 1)) +  # or percent_format()
  coord_flip() +
  labs(title = "Who makes decisions on data collection, processing, storage and transmission",
    x = "County Department",
...
standardize_law <- function(x) {
  ...
  x <- gsub("Oral Surveys|Online Surveys|Offline Surveys","Surveys",ignore.case = TRUE, x)
