...
# library(knitr)
# 
# kable(topic_labels, caption = "🏷️ Final Topic Labels Summary")
# 
# install.packages("gt")
# library(gt)
# 
# topic_labels %>%
#   gt() %>%
#   tab_header(title = "🏷️ Final Topic Labels Summary")
...
#   # If it mentions "Constitution", force to canonical label
#   x <- ifelse(str_detect(x, regex("Constitution", ignore_case = TRUE)),"Constitution of Kenya 2010", x)
#   # Other standardizations
#   x <- ifelse(str_detect(x, regex("DPA|data protection", ignore_case = TRUE)),"Data Protection Act 2019", x)
#   x <- ifelse(str_detect(x, regex("county government", ignore_case = TRUE)),"County Government Act", x)
#   x <- ifelse(str_detect(x, regex("county attorney", ignore_case = TRUE)),"County Attorney Act 2020", x)
#   x <- ifelse(str_detect(x, regex("Ppda", ignore_case = TRUE)),"Public Procurement & Disposal Act (PPDA) 2015",x)
# 
#   str_trim(x)
# }
# responses_cleaned <- standardize_law(responses)
...
law_split <- law_df2 %>%
  separate_rows(response, sep = ",") %>%   # split by comma or 'and'
...
dd <- open %>%
  select(department = `County Department`)
...
dept_combined <- dd %>%
  group_by(department) %>%
  summarise(all_laws = paste(response, collapse = ", "))

# 3. Split multiple laws into separate rows
dd_long <- dept_combined %>%
  separate_rows(all_laws, sep = ",") %>%
  mutate(all_laws = str_trim(all_laws)) %>%
  filter(!str_detect(all_laws, "N/A|Not Sure|No Law At The Moment")) %>%
  filter(all_laws != "")

law_counts_by_dept <- dd_long %>%
  count(department, all_laws, sort = TRUE)

df_prop <- law_counts_by_dept |>
  group_by(department) |>
  mutate(Prop = n / sum(n, na.rm = TRUE)) |>
  ungroup()

p_norm <- ggplot(df_prop, aes(x = department, y = Prop, fill = all_laws)) +
  geom_col(width = 0.85, color = "grey20", linewidth = 0.2) +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  coord_flip() +
  labs(
    title = "Method mix per department (normalized)",
    x = NULL, y = "Share within department"
  ) +
...
# 1. Load responses
responses <- str_to_title(open$`What are the primary data storage methods at your County`)

# 2. Standardization function

standardize_law <- function(x) {
  x <- gsub("\\/|\\#|\\?|\\*|\\.", "", x)
  x <- gsub("Filling|Filings|Files", "Filing", ignore.case = TRUE, x)
  x <- gsub("Hard Discs|Hard-Drives|","Hard Drives",ignore.case = TRUE,x)
  x <- gsub("Registry Filing","Registers",ignore.case = TRUE,x)
  x <- gsub("Hard Copy","Physical",ignore.case = TRUE,x)
  x <- gsub("Computer Storage Systems","Computers",x)
  x <- gsub("and",",",ignore.case = TRUE,x)
  x <- gsub("\\s*,\\s*|,\\s*", ",", x)
  str_trim(x)
}
...
responses_cleaned <- clean_responses(responses)
