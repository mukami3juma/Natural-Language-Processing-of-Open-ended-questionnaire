...
clean_responses <- function(responses) {
  ...
  standardize_law <- function(x) {
    ...
    x <- str_remove_all(x, "[[:punct:]]")  # Remove all punctuation
    x <- gsub("Questionare|Questionnaire|Questionares|Questionnaires", "Questionnaires", x, ignore.case = TRUE)
    x <- gsub("Discussion|Discussions", "Discussions", x, ignore.case = TRUE)
    x <- gsub("focused", "Focus", x, ignore.case = TRUE)
    x <- gsub("\\band\\b", ",", x, ignore.case = TRUE)  # Only replace "and" as a word
    x <- gsub("\\s*,\\s*|,\\s*", ",", x)
    str_trim(x)
  }
...
dd <- open %>%
  select(department = `County Department`, method = `What are the primary data collection methods at your County`) %>%
  filter(!is.na(method)) %>%
  mutate(method = str_to_title(method))

clean_responses(dd$method)
